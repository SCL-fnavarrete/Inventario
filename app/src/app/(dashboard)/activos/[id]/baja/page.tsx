...
export default function BajaActivoPage() {
  ...
  const hasActiveMaintenance = asset?.maintenances?.some((m) => m.estado === 'pendiente' || m.estado === 'en_proceso') ?? false;
  const canProceed = !hasActiveAssignment && !hasActiveMaintenance && asset?.estado !== 'vendido' && asset?.estado !== 'baja';
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`/api/activos/${id}/baja`, {
        method: 'POST',
        ...
        <div className="bg-white rounded-lg shadow p-6 space-y-4">
          ...
          {asset.estado === 'baja' && (
            <div className="bg-gray-50 border rounded-lg p-4 text-gray-600">
              Este activo ya está dado de baja.
            </div>
          )}
      ...
      {step === 2 && (
        <div className="bg-white rounded-lg shadow p-6 space-y-4">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm text-amber-700">
            <strong>Atención:</strong> Esta acción cambiará el estado del activo a &quot;Baja&quot;. Solo podrá venderse después.
          </div>
